...
class TrackGroupChatManager(GroupChatManager):
    def _process_received_message(self, message, sender, silent):
        ...
        with tab1:
            if ( ('UserProxyAgent' in str(sender)) and not(('exitcode' in str(message)) or ('if the task is done' in str(message)))  ):            
                with st.chat_message('Requestor', avatar="👨🏻‍💼"):
                    st.markdown(''' :blue[{}]'''.format(message))
            
            elif( ('AssistantAgent' in str(sender)) and not(('pandas' in str(message)) or ('execution' in str(message)) or ('python' in str(message)))  ):    
